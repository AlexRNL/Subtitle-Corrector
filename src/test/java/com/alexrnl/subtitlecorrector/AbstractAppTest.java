...
public class AbstractAppTest {
	...
	public void testAddStrategy () {
		...
		when(mockedStrategy.getName()).thenReturn("mockedStrategy");
		...
		assertEquals(new HashSet<>(Arrays.asList(mockedStrategy.getName())), newStrategies.keySet());
		
		final Strategy duplicateNameStrategy = mock(Strategy.class);
		when(duplicateNameStrategy.getName()).thenReturn("mockedStrategy");
